...
create policy sale_items_tenant_isolation on public.sale_items
  for all to authenticated
  using (exists (select 1 from public.sales s where s.id = sale_id and s.tenant_id = private.current_tenant_id()))
  with check (exists (select 1 from public.sales s where s.id = sale_id and s.tenant_id = private.current_tenant_id()));
...
create policy purchase_items_tenant_isolation on public.purchase_items
  for all to authenticated
  using (exists (select 1 from public.purchases p where p.id = purchase_id and p.tenant_id = private.current_tenant_id()))
  with check (exists (select 1 from public.purchases p where p.id = purchase_id and p.tenant_id = private.current_tenant_id()));
...
create policy sales_document_items_tenant_isolation on public.sales_document_items
  for all to authenticated
  using (exists (select 1 from public.sales_documents d where d.id = sales_document_id and d.tenant_id = private.current_tenant_id()))
  with check (exists (select 1 from public.sales_documents d where d.id = sales_document_id and d.tenant_id = private.current_tenant_id()));
