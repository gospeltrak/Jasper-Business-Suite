...
create table if not exists public.tenant_subscription_renewals (
  id uuid primary key default gen_random_uuid(),
  tenant_id uuid not null references public.tenants(id) on delete cascade,
  package_id text not null check (package_id in ('ruby', 'diamond', 'tanzanite')),
  ...
  renewal_source text not null check (renewal_source in ('payment_approval', 'super_admin_grant')),
  idempotency_key text not null,
  payment_proof_id uuid references public.tenant_payment_proofs(id) on delete set null,
...
create index if not exists tenant_subscription_renewals_tenant_created_idx
  on public.tenant_subscription_renewals (tenant_id, created_at desc);

alter table public.tenant_subscription_renewals enable row level security;
alter table public.tenant_subscription_renewals force row level security;

drop policy if exists tenant_subscription_renewals_platform_admin_read
  on public.tenant_subscription_renewals;
create policy tenant_subscription_renewals_platform_admin_read
  on public.tenant_subscription_renewals
  for select
  to authenticated
  using (private.is_platform_admin());

drop function if exists public.activate_tenant_package_period(uuid, text, integer, text, boolean, uuid);

create function public.activate_tenant_package_period(
  p_tenant_id uuid,
  p_package_id text,
