-- Fix the Branch Switcher / header showing the literal placeholder
-- "Primary Branch" instead of the tenant's real business name.
--
-- public.list_current_user_branches() (compatibility-primary case, i.e.
-- every tenant that has not yet provisioned a physical branch row) and
-- private.ensure_physical_primary_branch() (default name used when a
-- physical Primary Branch row is first provisioned) both hard-coded the
-- literal string 'Primary Branch' into the branchName/branch_name field
-- that the client renders as the primary label. The real tenant/business
-- name was only available in the secondary `businessName` field, which
-- the Branch Switcher does not use as its main label.
--
-- Repair: branchName / branch_name now defaults to the tenant's real name
-- (same coalesce fallback already used for businessName: tenant.name,
-- falling back to 'Business' only if the tenant has no name at all),
-- so the switcher's primary label is the real business name instead of a
-- placeholder. `isDefault` / `isCompatibilityPrimary` continue to be
-- returned unchanged, so the client's separate "Main Branch" badge is
-- unaffected. No schema change, no table change, no other function or
-- endpoint touched, no data migration of already-created physical
-- branches (an existing branch literally named "Primary Branch" by a
-- prior default keeps that name until a user renames it via Branch
-- settings - this migration only changes the default used going forward
-- and for the virtual compatibility-primary branch, which has no stored
-- row to migrate).
