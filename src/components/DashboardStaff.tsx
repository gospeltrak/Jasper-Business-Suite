...
export default function DashboardStaff({
  ...
  useEffect(() => {
    // Poll local storage to simulate "real-time" status indicator for staff
    const checkStatuses = () => {
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'list' && (
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
          <h3 className="font-bold text-slate-800 text-sm mb-4">Registered Staff</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {staffList.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-6 text-center text-slate-400 italic">No staff registered.</td>
                  </tr>
                ) : (
                  staffList.map(staff => {
          ...
          <div className="grid grid-cols-1 gap-4">
            {staffList.length === 0 ? (
              ...
              staffList.map(staff => {
                ...
                const deliveryMoney Earned = staffDeliveries.reduce((acc, d) => acc + (Number(d.deliveryCost) || 0), 0);
                const totalMoney Earned = revenueGenerated + deliveryMoney Earned;
                  ...
                  <div key={staff.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 border border-slate-200 rounded-2xl hover:bg-slate-50/50 transition-colors gap-4">
                    ...
                    <div className="grid grid-cols-3 gap-6 shrink-0">
                      ...
                      <div>
                        ...
                        <span className="text-lg font-black font-mono text-emerald-700">{currency}{totalMoney Earned.toLocaleString()}</span>
                      </div>
                      <div>
